...
namespace PhotoVs.Graphics.TextureAtlases
{
    public static class TextureAtlasExtensions
    {
        ...
        {
            Draw(spriteBatch, textureRegion, position, color, 0, Vector2.Zero, Vector2.One, SpriteEffects.None, 0,
                clippingRectangle);
        }
            ...
            Rectangle destinationRectangle, Color color, Rectangle? clippingRectangle = null)
        {
            ...
            if (ninePatchRegion != null)
                ...
                Draw(spriteBatch, textureRegion.Texture, textureRegion.Bounds, destinationRectangle, color,
                    clippingRectangle);
        }
            ...
            for (var i = 0; i < sourcePatches.Length; i++)
            {
                ...
                else
                {
                    if (destinationPatch.Width > 0 && destinationPatch.Height > 0)
                        spriteBatch.Draw(ninePatchRegion.Texture, sourceRectangle: sourcePatch,
                            destinationRectangle: destinationPatch, color: color);
                }
            }
        }

        public static void Draw(this SpriteBatch spriteBatch, Texture2D texture, Rectangle sourceRectangle,
            Rectangle destinationRectangle, Color color, Rectangle? clippingRectangle)
        {
            if (clippingRectangle.HasValue)
            {
        ...
        {
            var left = clippingRectangle.Left < destinationRectangle.Left
                ? destinationRectangle.Left
                : clippingRectangle.Left;
            var top = clippingRectangle.Top < destinationRectangle.Top
                ? destinationRectangle.Top
                : clippingRectangle.Top;
            var bottom = clippingRectangle.Bottom < destinationRectangle.Bottom
                ? clippingRectangle.Bottom
                : destinationRectangle.Bottom;
            var right = clippingRectangle.Right < destinationRectangle.Right
